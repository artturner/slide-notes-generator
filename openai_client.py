...
# Note: Install openai package with: pip install openai
try:
    import openai
...
class OpenAIClient:
    ...
    def generate_bullet_points(self, section_content: str, heading: str, 
        ...
        """
        Generate bullet points for a textbook section using OpenAI
...
Generate the bullet points now:"""
        ...
        try:
            ...
            print(f"\n--- DEBUG: Raw OpenAI response for '{heading}' ---")
            print(f"Response text: {response_text}")
            print("--- END DEBUG ---\n")
            
            # Parse the response
            bullet_points = self._parse_bullet_response(response_text, max_words_per_bullet)
    ...
    def _parse_bullet_response(self, response_text: str, max_words: int) -> List[str]:
        """Parse OpenAI response to extract valid bullet points"""
        bullets = []
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python openai_client.py <test_content>")
        print("Example: python openai_client.py 'The New Deal was a series of programs...'")
        sys.exit(1)
